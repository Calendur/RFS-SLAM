...
class MeasurementModel_VictoriaPark : public MeasurementModel<Pose2d, Landmark3d, Measurement3d>
{
  public:
     ...
     * with the Jacobian w.r.t. the landmark position \param[out] jacobian_wrt_pose if not NULL, the pointed-to matrix
     * is overwritten with the Jacobian w.r.t. the robot pose by the Jacobian of the measurement model,
     * \f$\mathbf{H}\f$, evaluated at \f$\mathbf{x}\f$ and \f$\mathbf{m}\f$ \return true if a valid measurement is
     * produced
     */
    bool measure(const Pose2d &pose,
                 ...
                 Eigen::Matrix3d *jacobian_wrt_pose = NULL);
    ...
    /**
     * \brief This function calculates the probability of detection (using probabilityODetection2) in several places to
     * determine whether it is both zero and nonzero in the landmark's neighborhood, if it is isCloseToSensingLimit is
     * set to 1. \param[in] pose robot pose \param[in] landmark landmark position \param[out] isCloseToSensingLimit true
     * if landmark is close to the sensing limit \return probability of detection
     */
    double probabilityOfDetection(const Pose2d &pose, const Landmark3d &landmark, bool &isCloseToSensingLimit);
     ...
     */
    double probabilityOfDetection2(const Pose2d &pose, const Landmark3d &landmark, bool &isCloseToSensingLimit);
    ...
    struct Config
    {
        std::vector<double> probabilityOfDetection_; /**< Array containing probabilities of detection for a circle given
                                                        the amount of points that should show up on the scan */
        double expectedClutterNumber_; /**< Expected number of clutter measurements, the intensity is asummed constant
                                          over the field of view of the robot */
