...
CostMatrixGeneral::CostMatrixGeneral(double **&C, unsigned int nR, unsigned int nC)
  : nR_(nR)
  , nC_(nC)
  , components_i(NULL)
  , components_j(NULL)
...
unsigned int CostMatrixGeneral::partition()
{
    ...
    ::boost::add_edge(
        nR_ + nC_ - 1, nR_ + nC_ - 1, G);  // add edge to self so the graph has the desired number of vetices
...
bool CostMatrixGeneral::getPartition(int p,
                                     double **&Cp,
                                     unsigned int &nRows,
                                     unsigned int &nCols,
                                     unsigned int *&row_indices,
                                     unsigned int *&col_indices,
                                     bool allocateExtendedMatrix)
{
    if (components_mat == NULL)
    {
        Cp          = NULL;
...
CostMatrix::CostMatrix(double **&C, int nDim)
  : CostMatrixGeneral(C, nDim, nDim)
  , n_(nDim)
  , C_reduced_(NULL)
  , n_reduced_(0)
...
int CostMatrix::getCostMatrixReduced(
    double **&C, int *&fixedAssignments, double *fixedScore, int *&i_remap, int *&j_remap)
{
